// Package authsig holds shared signatures for recognizing login/SSO walls from
// a URL or a response body alone, without loading a page in a browser.
//
...
// spends a browser on an authentication page; and the HTTP requester's redirect
// policy uses LooksLikeLoginURL to stop a chain at an authentication wall
// instead of scanning the identity provider's login page as if it were the
// target. Keeping the tables here gives every caller one source of truth
// instead of duplicating the IdP list.
//
// It lives outside pkg/spitolas (where it started) because pkg/http is now a
// caller: a browser-crawler subpackage is the wrong dependency for the core
// transport, and this package imports nothing but the standard library.
package authsig
...
// loginIDPSuffixes is loginIDPHosts pre-prefixed with a dot, for the
// parent-suffix test. Built once: the concatenation used to happen inside the
// match loop, which cost one allocation per provider per call — and since the
// transport's redirect policy started asking this question, "per call" means
// per redirect hop of every request.
var loginIDPSuffixes = func() []string {
	out := make([]string, len(loginIDPHosts))
	for i, h := range loginIDPHosts {
		out[i] = "." + h
	}
	return out
}()
...
func LooksLikeLoginURL(u *url.URL) bool {
	...
	for i, idp := range loginIDPHosts {
		if host == idp || strings.HasSuffix(host, loginIDPSuffixes[i]) {
			return true
		}
	}
