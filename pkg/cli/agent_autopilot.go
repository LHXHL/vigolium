...
func init() {
	...
	f.BoolVarP(&autopilotStateless, "stateless", "S", false, "Run the whole autopilot into a throwaway temporary database (your project DB is left untouched), then materialize --format outputs from it. Mirrors 'vigolium scan -S'. Not valid with --db or --resume; --db-isolate is ignored alongside it.")
