...
var scanExamples = FormatExamples(
	...
	"vigolium scan -t https://example.com --discover --discovery-wordlist ~/.vigolium/wordlists/fuzz.txt",
...
var trafficExamples = FormatExamples(
	...
	"# Copy a whole glob into Burp's Site map — streams one file at a time (no merge),",
	"# so peak memory is one file rather than the whole set, with per-file progress",
	"vigolium traffic -S --glob-db 'scans/*.sqlite' --save-to-burp -B http://127.0.0.1:9009 --all",
	"",
...
var runExamples = FormatExamples(
	...
	"# Redirect policy (probe defaults to same-apex; hops are recorded, minus",
	"# canonical ones - a scheme upgrade or trailing slash collapses into its",
	"# destination row - and capped at 4 rows per chain, ends kept)",
	"vigolium run probe -T hosts.txt --redirect-mode any --record-redirect-chain",
	"# Every mode stops at a login/SSO wall and records the 3xx; chain_truncated",
	"# marks a row whose destination is not the row after it",
	...
	"# Discovery with custom fuzz wordlist",
	"vigolium run discover -t https://example.com --discovery-wordlist ~/.vigolium/wordlists/fuzz.txt",
	"# Run browser-based spidering",
